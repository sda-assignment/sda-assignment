...
public class AdminRefundController {
    ...
    public Response acceptRefund(int rid) throws EntitySaveException {
        ...
        ArrayList<User> users = userRelation.update(
                u -> new User(u.email, u.username, u.password, u.isAdmin, u.wallet + targetRefund.amount),
        ...
        if (users.size() > 0) {
            ...
            transactionRelation
                    .insert(new Transaction(Util.incrementOrInitialize(transactionRelation.selectMax(t -> t.id)),
                            updatedUser.email, LocalDateTime.now(), targetRefund.amount, TransactionType.REFUND));
        }
