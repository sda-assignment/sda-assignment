...
public class RefundController {
    ...
    public Response evaluateRefund(boolean status, int rid) throws EntitySaveException {
        if (status == true) {
            ...
            return new Response(true, "Refund got accepted, returning funds to wallet");
        } else {
            refundRelation.update(
                    r -> new RefundRequest(r.id, r.amount, r.serviceName, RefundRequestStatus.REJECTED, r.userEmail),
                    r -> r.id == rid);
            return new Response(false, "Refund got Rejected");
        }
    }
